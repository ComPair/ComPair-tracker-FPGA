...
int parse_args(int argc, char **argv) {
    ...
    int n_pulses = -1;
    CalCtrl calctrl;
    calctrl.load_current_settings(); // Read current settings.
    for (int i=0; i<argc; i++) {
        if (strcmp("--help", argv[i]) == 0 || strcmp("-h", argv[i]) == 0) {
            ...
            if (++i >= argc) {
                std::cerr << "ERROR: Cal dac value not provided." << std::endl;
                return CAL_PARSE_ARGS_ERR;
            }
            if (calctrl.set_cal_dac((u32)atoi(argv[i])) != 0) {
                std::cerr << "ERROR: Cal dac value not in range. Probably too large." << std::endl;
                return CAL_DAC_VAL_ERR;
            }
